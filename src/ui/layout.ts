...
type CommandDockSection = 'actions';
...
function ensureCommandDockSection(
  ...
  element.dataset.hudCommandDockSection = section;
  element.classList.add('hud-command-dock__section', 'hud-command-dock__actions');
...
export function ensureHudLayout(overlay: HTMLElement): HudLayout {
  ...
  const commandDockActions = ensureCommandDockSection(anchors.commandDock, doc, 'actions');
  ...
  const rosterHud = overlay.querySelector<HTMLElement>('#resource-bar');
  if (rosterHud && rosterHud.parentElement !== regions.right) {
    regions.right.appendChild(rosterHud);
  }
